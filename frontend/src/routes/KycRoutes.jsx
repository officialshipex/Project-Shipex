...
const KycRoutes = () => {
  ...
  useEffect(() => {
    try {
      ...
      const getUser = async () => {
        const response = await axios.get("http://localhost:5000/v1/merchant/verfication/kyc", {
          headers: {
  ...
  const kycVerify = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5000/v1/merchant/verfication/kyc", {
        businesstype,
        companyName,
        gstNumber,
        address,
