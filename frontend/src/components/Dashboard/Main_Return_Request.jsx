  ...
  return (
    ...
    <div className="flex-1">
      ...
      <div className="bg-white shadow-md rounded-lg mb-4">
        <div className="border-b p-4 flex font-medium text-left bg-[#FAFAFA]">
          <input className=" accent-[#0CBB7D] border-4 border-green-500  focus:outline-none" type="checkbox" onChange={handleSelectAll} />
      ...
      <div className="space-y-4">
        {filteredOrders.length > 0 ? (
          filteredOrders.map(order => (
            <div key={order.id} className="bg-[#FAFAFA]  p-4 mb-4">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  <p className=" text-[#1A1A1A] text-[14px]">
                    ...
                    <span className="text-gray-600">{new Date(order.date).toLocaleDateString('en-GB', { year: 'numeric', month: 'long', day: 'numeric' })} | {new Date(order.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }).replace(':', '.')}</span> <br />
                    <span>CUSTOM</span> <br />
                ...
                <div className="flex-1">
                  <p className=" text-[#1A1A1A]">{order.buyerName}</p>
                  <p>{order.email}</p>
                </div>

                <div className="flex-1">
                  <p className=" text-[#1A1A1A]">{order.product}</p>
                  <p className="px-3 py-1 bg-[#FAFAFA] text-[#1B8D46] rounded text-sm shadow-md inline-block mt-1">Prepaid</p>
                </div>
