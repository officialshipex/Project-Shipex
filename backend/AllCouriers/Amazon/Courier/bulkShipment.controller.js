...
const createShipmentAmazon = async (serviceDetails, orderId, wh, walletId, charges) => {
    try {
      ...
      const { rate, requestToken,valueAddedServiceIds } = await checkAmazonServiceability("Amazon", payload);
      ...
      if (!result) {
        return { success: false, message: "Error creating shipment", error: response.data };
      }
  
      const trackingId = result.packageDocumentDetails[0].trackingId;
      console.log("tracking",trackingId)
      const base64Label = result.packageDocumentDetails[0].packageDocuments[0].contents;
      const labelBuffer = Buffer.from(base64Label, "base64");
      const labelKey = `labels/${Date.now()}_${currentOrder.orderId || "label"}.pdf`;
      ...
      console.error("❌ Error creating Amazon shipment:", error.response?.data || error.message);
      return {
        success: false,
