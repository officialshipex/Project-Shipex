        ...
        try {
          const order = new bulkOrdersCSV({
            ...
            orderDetails: {
              ...
              sellingPrice: parseFloat(row['*Selling Price(Per Unit Item, Inclusive of Tax)']),
              discount: parseFloat(row['Discount(Per Unit Item)']) || 0,
              shippingCharges: parseFloat(row['Shipping Charges(Per Order)']  || 0),
              codCharges: parseFloat(row['COD Charges(Per Order)']  || 0),
              giftWrapCharges: parseFloat(row['Gift Wrap Charges(Per Order)']  || 0),
            ...
            sendNotification: row['Send Notification(True/False)'].toLowerCase() === 'true',
            comment: row['Comment'] || '',
    ...
    const orderDocs = orders.map((row) => {
      return {
        ...
        buyer: {
          ...
          email: row['Email (Optional)'] || '',
          alternatePhone: row["Buyer's Alternate Mobile Number (Optional)"] || '',
        ...
        orderDetails: {
          ...
          masterSKU: row['*Master SKU'] || '',
          quantity: parseInt(row['*Product Quantity'] || 0),
          unitPrice: parseFloat(row['*Per Unit Price in INR (Inclusive of Tax)'] || 0),
          productDiscount: parseFloat(row['Product Discount (Per Unit Item) (Optional)'] || 0),
          hsnCode: row['HSN Code (Optional)'],
          taxRate: parseFloat(row['Tax Rate(percentage) (Optional)'] || 0),
          shippingCharges: parseFloat(row['Shipping Charges (Per Order) (Optional)'] || 0),
          giftWrapCharges: parseFloat(row['Gift Wrap Charges (Per Order) (Optional)'] || 0),
          transactionFee: parseFloat(row['Transaction Fee (Per Order) (Optional)'] || 0),
          totalDiscount: parseFloat(row['Total Discount (Per Order) (Optional)'] || 0),
        ...
        sendNotification: row['Send Notification (Yes/No) (Optional)'] === 'Yes',
