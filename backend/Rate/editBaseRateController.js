...
const editBaseRate = async (req, res) => {
    ...
    try {
        ...
        const prevRateCard = await BaseRate.findById(req.body.id);
        const newRateCard = req.body;
        ...
        let customPlan = await CustomRate.find({});
        for (let cp of customPlan) {
            for (let rc of cp.ratecards) {
                let rcard = await RateCard.findById(rc);
             
                if (rcard.courierServiceName === req.body.courierServiceName) {
        ...
        await BaseRate.findByIdAndUpdate(
            req.body.id, 
            {
        ...
        res.status(201).json({success:true,message:'succesfully updated'});    
    } catch (error) {
