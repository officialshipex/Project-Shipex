...
const getZone = async (fromPinCode, toPinCode, res) => {
    ...
    if (!fromPinCodeDetails || !toPinCodeDetails) {
        return res
            ? res.status(400).json({
                  message: "Pincode details not found, please enter valid pincode or try again later.",
              })
            : { error: "Pincode details not found, please enter valid pincode or try again later." };
    }
    ...
    if (
        ZONE_E_STATES.includes(toState) ||
        ZONE_E_STATES.includes(fromState)
    ) {
        return {
            zone: "zoneE",
            details: { fromPinCodeDetails, toPinCodeDetails },
